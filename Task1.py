...
def tau_AB_():
    """ Calculates tau_AB"""
    numerator = (f_st - f_()) * (eff_AB*H/(c_pt*T_0))+tau['t']*tau['lambda']*(1 + f_())
    denominator = tau['t']*tau['lambda']*(1 + f_()) + (f_st - f_())*tau['t']*tau['lambda']
    tau_AB = numerator/denominator
    print(f"tau_AB = {tau_AB}")
    return tau_AB #Actually supposed to calculate using f_st = f combustion things.

def pi_t(M0):
    pi_t = tau_t(M0) ** (gamma_t/((gamma_t-1)*e_t))
    print(f"pi_t = {pi_t}")
    return pi_t
...
def F(M0):
    ...
    Fcore = m0_(M0)*(a0/(1+alpha))*((1+f_()+f_AB_())*(np.sqrt(gamma_t*Rt*TC_ratio(M0)*gamma_c*Rc) * M9())-M0) # P0=P9, so last term all goes to 0.
    Fbypass = m0_(M0)*(alpha*a0/(1+alpha))*((1+f_B_())*(np.sqrt(gamma_t*Rt*TB_ratio(M0)*gamma_c*Rc) * M19(M0))-M0) # P0=P19, so last term all goes to 0.
...
plt.figure(figsize=(8, 5))
plt.xlabel('Mach Number (M0)')
plt.ylabel('Specific Fuel Consumption [m/s]')
plt.title('Specific Fuel Consumption vs Mach Number (M0)')
plt.grid(True)
plt.legend()
plt.tight_layout()
